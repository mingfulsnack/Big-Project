...
function FooterMusicPlayer({music}) {
    ...
    const handleAddToList = (event, newValue) => {
        dispatch({type:"show_bg",payload:isBg});
        localStorage.setItem("state",JSON.stringify(state));
    }

    useEffect(()=>{
        dispatch1(setBannerOpen(bannerToggle));
    ...
    useEffect(()=>{
        if (isNextClicked){
            ...
            dispatch1(setCurrentPlaying(playlists[currTrackId]));
    ...
    return (
        <div style={useStyle.component} className={"footer-player"}>
            ...
            <div>
                <ControlsToggleButton style={pointer} type={"add"}
                                      defaultIcon={<Add/>}
                                      changeIcon={<Add/>}
                                      onClicked={handleAddToList}/>
            </div>
        </div>
